...
{	while(fscanf(f,"%d %s %s %d %d %d %s %d %d %s\n",&(p.id),p.prenom,p.nom,&(p.d.j),&(p.d.m),&(p.d.a),p.bloc,&(p.chambre),&(p.tel),p.email)!=EOF)
	...
	if(b==0){
	fprintf(f,"%d %s %s %d %d %d %s %d %d %s\n",e.id,e.prenom,e.nom,e.d.j,e.d.m,e.d.a,e.bloc,e.chambre,e.tel,e.email);
...
if(f==NULL||g==NULL)
{
	...
	{
		if(p.id!=e.id)
			fprintf(g,"%d %s %s %d %d %d %s %d %d %s\n",p.id,p.prenom,p.nom,p.d.j,p.d.m,p.d.a,p.bloc,p.chambre,p.tel,p.email);
		else
			fprintf(g,"%d %s %s %d %d %d %s %d %d %s\n",p.id,e.prenom,e.nom,p.d.j,p.d.m,p.d.a,e.bloc,e.chambre,e.tel,e.email);
	}
...
gtk_list_store_set(store,&iter,ID,p.id,NOM_COMPLET,nom_complet,DATE_NAISSANCE,date_naissance,BLOC,p.bloc,CHAMBRE,chambre,TEL,p.tel,EMAIL,p.email,-1);
	}
	fclose(f);
